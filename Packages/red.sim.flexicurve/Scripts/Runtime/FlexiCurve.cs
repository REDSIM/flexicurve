    ...
    public class FlexiCurve : MonoBehaviour {
        ...
        public void OnValidate() {
            ...
            if (Points == null || Points.Length < 2) {
                Points = new Vector3[] { new Vector3(0, 0, 1), new Vector3(1, 0, 0) };
                Sags = new float[] { -0.25f };
            }
                ...
                for (int i = 0; i < elementPoints.Length; i++) {
                    ...
                    for (int l = 1; l < count; l++) {
                        ...
                        Vector2 shift = new Vector2(
                            (float)((float)(elementId + (GenerateWires ? _wireSegments.Length : 0)) % uvSideSize + LightmapPadding / 2) / uvSideSize,
                            (float)(Mathf.Floor((float)(elementId + (GenerateWires ? _wireSegments.Length : 0)) / uvSideSize) + LightmapPadding / 2) / uvSideSize
                            );
            ...
            if (_vertices.Count > 65535) Filter.sharedMesh.indexFormat = UnityEngine.Rendering.IndexFormat.UInt32;
            else Filter.sharedMesh.indexFormat = UnityEngine.Rendering.IndexFormat.UInt16;
