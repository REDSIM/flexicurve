...
namespace flexicurve
{
    ...
    public class BezierMesh
    {
        ...
        public BezierMesh(Vector3 a, Vector3 b, float sag, float radius, float spacing, int edges, float decimation,
            bool closedSides)
        {
        ...
        public void Recalculate(int offset)
        {
            ...
            if (Mathf.Abs(dir.x) < 0.00001f && Mathf.Abs(dir.z) < 0.00001f)
                tangent = Vector3.Cross(Vector3.forward, dir).normalized;
            ...
            for (int i = 0; i < pointsCount; i++)
            {
                ...
                if (i == 0) dirCurr = Vector3.Normalize(points[i + 1] - points[i]); // First point
                    ...
                    dirCurr = Vector3.Normalize(Vector3.Normalize(points[i + 1] - points[i]) +
                                                Vector3.Normalize(points[i] - points[i - 1])); // Other points
