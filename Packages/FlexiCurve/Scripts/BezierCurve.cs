        ...
        public Vector3[] GetUniformPointArray(float spacing)
        {
            ...
            List<Vector3>
                result = new List<Vector3>()
                    { dots[0] }; // List of all dots placed on equal distance from each other (first dot included)
            ...
            for (int i = 0; i < dots.Length - 1; i++)
            {
                ...
                else
                {
                    ...
                    } while (Vector3.Distance(currentPoint, dots[i + 1]) >=
                             spacing); // Iterating in case if the current raw segment is too long
